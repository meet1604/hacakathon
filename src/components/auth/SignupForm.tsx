...
export function SignupForm({ next = "/dashboard?welcome=1" }: Props) {
  const [state, action, isPending] = useActionState(signUpWithPassword, {
    error: null,
  });
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <form action={action} className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1.5">
          <label htmlFor="confirm" className="text-[14px] font-medium text-text-1">
            Confirm password
          </label>
          <input
            id="confirm"
      ...
      <p className="text-center text-[14px] text-text-2">
        ...
        <Link href={`/login${next && next !== "/dashboard?welcome=1" ? `?next=${encodeURIComponent(next)}` : ""}`} className="font-medium text-primary hover:underline">
          Sign in
        </Link>
      </p>
    </div>
